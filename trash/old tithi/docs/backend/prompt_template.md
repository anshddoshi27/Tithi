...
## Prompt Template (Tithi Execution Standard)

### 0. Mission Context & Alignment
- Briefly restate the mission and how this task supports it.
- Link relevant files: `TITHI_FRONTEND_DESIGN_BRIEF.md`, `backend_report.md`, `TITHI_DATABASE_COMPREHENSIVE_REPORT.md`.

### 1. Task Definition Block
- Copy the authoritative Task Definition Block here (or summarize if very long).

### 2. Confidence Assessment
- Confidence: <0–100%>
- If <100%: list uncertainties and blocking questions (do not proceed until resolved).

### 3. Inputs, Constraints, and Invariants
- Inputs:
- Outputs:
- Constraints: GDPR/PCI, RLS, idempotency, performance targets
- Invariants to uphold (business logic, safety, determinism)

### 4. Plan (Deterministic Implementation)
- Steps:
  1. ...
  2. ...
- Integration points and affected modules/files.

### 5. Schema & Migration Check (MANDATORY)
- Relevant tables/entities:
- Alignment checked against `all_migrations_consolidated.txt` and `TITHI_DATABASE_COMPREHENSIVE_REPORT.md`:
- Notes on RLS, constraints, indexes, functions involved:

### 6. Implementation
- Files to create:
  - `path/to/new_file` — purpose
- Files to edit:
  - `path/to/edited_file` — change summary
- Key decisions and rationale:

### 7. Observability, Safety, Idempotency
- Error model and codes:
- Metrics/logs emitted:
- Idempotency strategy (client_generated_id, headers, retries):
...
- Append to `backend_report.md`:
  - Step header, files created/edited, implementation details, issues resolved, testing, dependencies
...
- What was produced:
- How it meets the Task Definition Block:
- Known limitations (if any):

### 12. Summary
- 2–4 bullet points summarizing impact and next steps.

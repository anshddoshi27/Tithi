 ...
 * 
 * A proper visual calendar grid that displays horizontally with time slots and day columns.
 * Matches the target design with proper layout and styling.
 */
...
export const GridCalendar: React.FC<GridCalendarProps> = ({
  staffMembers,
  timeBlocks,
  onTimeBlockAdd,
  onTimeBlockUpdate,
  ...
  // Generate week days (January 15-21, 2024) to match target design
  ...
  // Sample appointments for Monday (January 15, 2024) to match target design
    ...
    return [
      {
        ...
        staff_name: 'Maria',
        day_of_week: 0, // Monday (index 0 in weekDays array)
        start_time: '10:00',
        end_time: '11:00',
        clientName: 'Sarah',
                      ...
                      >
                        {slotAppointments.length > 0 ? (
                          slotAppointments.map((timeBlock) => (
                            <div
                              key={timeBlock.id}
                              className="bg-gray-100 border border-gray-200 rounded p-1 text-xs cursor-move hover:bg-gray-200 transition-colors h-full flex flex-col justify-center"
                              style={{ 

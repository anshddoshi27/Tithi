...
interface GoLiveStepProps {
  ...
  payment: Pick<PaymentSetupConfig, "subscriptionStatus" | "trialEndsAt" | "nextBillDate">;
  onLaunch: () => void;
  onStartTrial: () => void;
  onBack: () => void;
}
    ...
    <div className="relative space-y-10" aria-labelledby="go-live-step-heading">
      ...
      <div className="rounded-3xl border border-white/10 bg-white/5 p-6 text-sm text-white/70">
        ...
        <ul className="mt-3 space-y-2 list-disc pl-5 text-left text-white/60">
          ...
          <li>
            Stripe handles payouts via Connect. Subscription is {formatSubscription(payment)} — manage it in
            Account.
          </li>
        </ul>
      </div>
...
function formatSubscription(payment: Pick<PaymentSetupConfig, "subscriptionStatus" | "trialEndsAt" | "nextBillDate">) {
  switch (payment.subscriptionStatus) {
    case "trial":
      return payment.trialEndsAt
        ? `on trial until ${formatDate(payment.trialEndsAt)}`
        : "on trial";
    case "active":
      return payment.nextBillDate
        ? `active (next bill ${formatDate(payment.nextBillDate)})`
        : "active";
    case "paused":
      return "paused (billing stopped)";
    case "canceled":
      return "canceled — subdomain deprovisions after retention";
    default:
      return "pending";
  }
}

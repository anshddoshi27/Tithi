...
function AvailabilityStep({
  ...
  const staffOptions = staff.filter((member) => serviceHasStaff(service.id, member.id, staff));
      ...
      {totalSlots === 0 ? (
        <div className="mt-10 rounded-3xl border border-white/10 bg-black/60 p-8 text-center text-white/60">
          ...
          <p className="mt-2 text-sm">
            ...
            <a href={`mailto:${staffEmailFallback(staff)}`} className="underline">
              {staffEmailFallback(staff)}
            </a>
            .
          </p>
        </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h2 className="mt-2 font-display text-3xl text-white">{service.name}</h2>
            <p className="mt-1 text-sm text-white/60">
              {formatDuration(service.durationMinutes)} · {formatcurrencyInline(listPriceCents)} — with{" "}
              {slot.staffName}
            </p>
          </div>
          <Button variant="ghost" onClick={onBack} className="inline-flex items-center gap-2 text-white/70">
...
function formatcurrencyInline(cents: number) {
...
function serviceHasStaff(serviceId: string, staffId: string, staff: FakeBusinessWorkspace["staff"]) {
  return staff.some((member) => member.id === staffId);
}

function staffEmailFallback(staff: FakeBusinessWorkspace["staff"]) {
  return staff[0]?.name
    ? `${staff[0].name.split(" ")[0].toLowerCase()}@${staff[0].name.split(" ")[0].toLowerCase()}.tithi`
    : "support@tithi.com";
}

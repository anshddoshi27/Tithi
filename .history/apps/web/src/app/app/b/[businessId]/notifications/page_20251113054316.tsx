        ...
        {workspace.notifications.map((template) => (
          <article key={template.id} className="rounded-3xl border border-white/15 bg-white/5 p-6">
            <header className="flex items-center justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-wide text-white/50">{template.channel}</p>
              ...
              <div className="mt-3 flex flex-wrap gap-2 text-xs text-white/60">
                {PLACEHOLDERS.map((placeholder) => (
                  <button
                    ...
                    type="button"
                    className="rounded-full border border-white/15 bg-white/5 px-3 py-1 transition hover:border-white/30 hover:text-white"
                    onClick={() =>
...
function NotificationPreview({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur">
      <div className="relative w-full max-w-2xl rounded-3xl border border-white/10 bg-[#060F28] px-6 py-8 text-white shadow-[0_60px_160px_rgba(4,12,35,0.55)]">
        <button
          type="button"
          className="absolute right-6 top-6 rounded-full border border-white/20 bg-white/5 px-3 py-1 text-xs uppercase tracking-wide text-white/60 transition hover:text-white"
          onClick={onClose}
        >
          Close
        </button>

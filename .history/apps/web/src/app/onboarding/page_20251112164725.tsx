...
export default function OnboardingPage() {
  ...
  const handleWebsiteNext = (values: Parameters<typeof onboarding.saveWebsite>[0]) => {
    onboarding.saveWebsite(values);
    if (values.status === "reserved") {
      const url = `https://${values.subdomain}.tithi.com`;
    ...
    case "goLive":
      content = (
        <GoLiveStep
          ...
          staff={onboarding.team}
          categories={onboarding.services}
          bookingUrl={onboarding.bookingUrl ?? `https://${onboarding.website.subdomain || "yourbusiness"}.tithi.com`}
